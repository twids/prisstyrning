...
- Determine if the request is a net-new feature vs bugfix/chore/refactor/docs
- If it is a new feature and current branch is `master`, create and switch to `feat/<short-task-slug>` before implementation
- If already on a non-`master` branch, continue on that branch unless user requests otherwise
- Do not ask the user to create branches manually for routine feature work
...
6. **Resolve Open Questions (Conditional Stop)**: If the plan has unresolved decisions, pause for user answers. If there are no open questions, continue autonomously.

7. **Write Plan File**: Write the plan to `<plan-directory>/<task-name>-plan.md` (using the configured plan directory).

8. **Prepare Branch for Implementation**:
   - Determine whether the work is a new feature.
   - If new feature and current branch is `master`, create and switch to `feat/<short-task-slug>` before Phase 2.
   - If not a new feature, or already on non-`master`, continue on the current branch.
...
### 2C. Autonomous Commit and Phase Record
1. **Present Non-Blocking Summary**:
...
3. **Commit Autonomously**:
   - Generate the commit message following <git_commit_style_guide>
   - Create the git commit directly (do not ask the user to commit)
   - Commit only files relevant to the completed phase
   - If there are no staged changes, state that explicitly and continue

4. **Continue Automatically**: Proceed to next phase without waiting for routine confirmation.
...
Atlas must execute git commits autonomously between phases. Do not ask the user to run commit commands unless a hard blocker prevents committing.
</git_commit_style_guide>

<stopping_rules>
CRITICAL PAUSE POINTS - You must stop and wait for user input at:
1. If the plan has unresolved open questions requiring user decisions
2. If a hard blocker occurs that cannot be resolved autonomously
3. After plan completion document is created

DO NOT pause between phases for routine implementation, review, and commit progression.
</stopping_rules>
