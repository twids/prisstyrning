...
async function loadScheduleHistory() {
  ...
  try {
    ...
    history.forEach((entry, idx) => {
      ...
      header.innerHTML = `<span style="font-weight:bold;">${localizedDate}</span><span style="font-size:.9em;opacity:.7;">${localizedTimestamp}</span><button style="margin-left:auto;">${idx === 0 ? '▼' : '▶'}</button>`;
