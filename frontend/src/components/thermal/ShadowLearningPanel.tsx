...
export default function ShadowLearningPanel({ versions, failed, refresh }: { versions: ShadowLearningVersion[]; failed: boolean; refresh: () => Promise<unknown> }) {
  ...
  return <Paper component="section" variant="outlined" aria-labelledby="shadow-learning-heading" sx={{ p: { xs: 2, sm: 3 } }}>
    <Stack spacing={2}>
      ...
      <Typography variant="caption">Uppdateras även varje timme i Shadow. Kritiska rum och rum med positiv vikt behöver giltiga eller uttryckligen antaget oförändrade värden från en aktuell HA-avläsning. Rapportålder ensam blockerar inte startprognosen; en extra livsteckenssensor behövs inte. Ett icke-kritiskt rum med vikt 0 används bara för uppföljning och blockerar inte startprognosen. Ingen träning eller aktivering i andra driftlägen.</Typography>
      ...
      {!failed && !version && <Typography>Ingen startprognos har sparats ännu. Välj minst ett kritiskt rum och invänta en aktuell HA-avläsning med användbar rumstemperatur. Temperaturen behöver inte ha ändrats. Saknad värmedata hindrar inte den enkla temperaturbaslinjen.</Typography>}
      {!failed && version && <>
        ...
        <Typography>Version {version.id} · utfärdad {formatDateTime(version.issuedAtUtc)} · {version.learning.samples} användbara insamlingspunkter.</Typography>
        {version.learning.independentSamples != null && version.learning.assumedSamples != null
          ? <Typography>{version.learning.assumedSamples} punkter med antagen temperatur · {version.learning.independentSamples} oberoende rapporterade observationer. Upprepade avläsningar av samma rapport räknas inte som nya observationer.</Typography>
          : <Typography>Denna äldre version saknar separat redovisning av oberoende rapporter och antagna värden.</Typography>}
        {version.learning.initialTemperatureAssumed && <Alert severity="warning">Starttemperaturen är antagen oförändrad. Prognosen kan visas, men antagna värden räknas inte som uppmätt utfall, förbättrat prognosfel eller godkännande för aktiv styrning.</Alert>}
        ...
        <Typography variant="body2">Streckad linje: prognos utfärdad i förväg. Punkter: senare, nya rapporterade rumsvärden, viktade och korrigerade för rumsoffset, under den verkliga styrningen. Antagna eller återlästa värden visas inte som uppmätt utfall. Ingen simulerad besparing redovisas.</Typography>
