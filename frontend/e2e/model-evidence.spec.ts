...
test('modellvyn skiljer modellbevis från aktiv styrning och återhämtar sig säkert från läsfel', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText(/Kodrevision vid träning: 0123456789ab/)).toBeVisible();
  await expect(page.getByText(/Vanliga programuppdateringar kräver inte omträning/)).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(await page.evaluate(() => window.innerWidth));
